...
namespace LaserGRBL.UserControls.DockingManager
{
	internal class VS2005AutoHideStrip : AutoHideStripBase
	{
		...
		private void DrawTab(Graphics g, TabVS2005 tab)
		{
			...
			using (LinearGradientBrush B = new LinearGradientBrush(rectTabOrigin, startColor, endColor, gradientMode))
				g.FillPath(B, path);
			...
			{
				if (dockState == DockState.DockLeftAutoHide || dockState == DockState.DockRightAutoHide)
					g.DrawString(content.DockHandler.TabText, TextFont, B, rectText, StringFormatTabVertical);
				else
					g.DrawString(content.DockHandler.TabText, TextFont, B, rectText, StringFormatTabHorizontal);
			}
			// Set rotate back
