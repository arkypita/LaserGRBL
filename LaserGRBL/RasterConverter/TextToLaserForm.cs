...
namespace LaserGRBL.RasterConverter
{
	...
	public partial class TextToLaserForm : Form
	{
		GrblCore mCore;
		ImageProcessor IP;
		bool preventClose;
		...
		public TextToLaserForm(GrblCore core, string filename, bool append)
		{
			InitializeComponent();
			mCore = core;
			...
			using (InstalledFontCollection fontsCollection = new InstalledFontCollection())
			{
				...
				{
					FontViewModel vm = new FontViewModel(font);

					CbFont.Items.Add(vm);
					fonts.Add(vm);

				}
				if (CbFont.Items != null && CbFont.Items.Count > 0)
		...
		{
			using (TextToLaserForm f = new TextToLaserForm(core, filename, append))
				f.ShowDialog(parent);
		}
		...
		private void RasterToLaserForm_Load(object sender, EventArgs e)
		{
			...
			rtbxText.Text = Settings.GetObject<string>("LastText", "LaserGBRL");
			string lastUsedFontFanilyName = Settings.GetObject<string>("LastUsedFamily", "Arial");
			for(int i = 0;i < CbFont.Items.Count;i++)
            {
				var vm = (FontViewModel)CbFont.Items[i];
				if(vm.Family.Name == lastUsedFontFanilyName)
                {
		...
		void RasterToLaserFormFormClosing(object sender, FormClosingEventArgs e)
		{
			...
			else
			{
				...
				Settings.SetObject("LastText", rtbxText.Text);
				Settings.SetObject("LastUsedFamily", SelectedFamily.Name);
			}
		}
		...
		private void richTextBox1_TextChanged(object sender, EventArgs e)
		{
			...
			if (!rtbxText.Text.IsNullOrEmpty())
			{
				ReDrawText();
			}
		}
		FontFamily SelectedFamily = FontFamily.GenericSansSerif;
		int bmpPreviewWith = 100;
		int bmpPreviewHeight = 50;
		private void ReDrawText()
		{

			FontStyle style = FontStyle.Regular;
			if (cbxFontUnderline.Enabled && cbxFontUnderline.Checked)
			{
				style |= FontStyle.Underline;
			}
			if (cbxFontBold.Enabled && cbxFontBold.Checked)
			{
				style |= FontStyle.Bold;
			}
			if (cbxFontStrikeOut.Enabled && cbxFontStrikeOut.Checked)
			{
				style |= FontStyle.Strikeout;
			}

			if (cbxFontItalic.Enabled && cbxFontItalic.Checked)
			{
				style |= FontStyle.Italic;
			}

			Font f = new Font(SelectedFamily, 100, style);

			Bitmap sizeB = new Bitmap(100, 100);
			sizeB.SetResolution(300, 300);
			Graphics sg = Graphics.FromImage(sizeB);
			SizeF size = new SizeF();
			if (!rtbxText.Text.IsNullOrEmpty())
			{
				size = sg.MeasureString(rtbxText.Text, f);
				bmpPreviewWith = ((int)size.Width) + 3;
				bmpPreviewHeight = ((int)size.Height) + 3;
			}
			sg.Dispose();
			sg = null;
			sizeB.Dispose();
			...
			using (Bitmap bmp = new Bitmap(bmpPreviewWith, bmpPreviewHeight))
			{
				bmp.SetResolution(300, 300);
				Graphics dg = Graphics.FromImage(bmp);

				dg.TextRenderingHint = TextRenderingHint.AntiAlias;
				dg.SmoothingMode = SmoothingMode.AntiAlias;
				dg.InterpolationMode = InterpolationMode.HighQualityBicubic;
				dg.PixelOffsetMode = PixelOffsetMode.HighQuality;
				dg.DrawString(rtbxText.Text, f, Brushes.Black, 0, 0);
				bmp.Save(fileName);
				dg.Dispose();
				IP.ReloadImage();
				IP.Refresh();
		...
		private void CbFont_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			SelectedFamily = ((FontViewModel)CbFont.SelectedItem).Family;
			cbxFontBold.Enabled = SelectedFamily.IsStyleAvailable(FontStyle.Bold);
			cbxFontItalic.Enabled = SelectedFamily.IsStyleAvailable(FontStyle.Italic);
			cbxFontStrikeOut.Enabled = SelectedFamily.IsStyleAvailable(FontStyle.Strikeout);
			cbxFontUnderline.Enabled = SelectedFamily.IsStyleAvailable(FontStyle.Underline);
			ReDrawText();
